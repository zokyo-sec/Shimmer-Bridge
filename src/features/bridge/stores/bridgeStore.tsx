...
import {roundUpToDecimals} from '../sdk/roundUpToDecimals';
...
export class BridgeStore {
  ...
  get maxAmount(): CurrencyAmount | undefined {
    ...
    if (isEvmChainId(srcChainId)) {
      if (srcBalance.currency instanceof Token) return srcBalance;
    ...
    yield(
      (this.promise.messageFee = fromPromise(
        transferApi.getMessageFee(srcCurrency, dstCurrency, adapterParams).then((fee) => {
          if (srcCurrency.chainId === ChainId.SHIMMER) {
            return roundMessageFee(fee);
          }
...
function isValidPair(srcCurrency: Currency, dstCurrency: Currency): boolean {
  ...
  return bridgeStore.apis.some((api) => api.supportsTransfer(srcCurrency, dstCurrency));
}
